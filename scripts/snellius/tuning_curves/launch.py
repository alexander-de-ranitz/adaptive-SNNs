...
def create_jobs():
    parser = argparse.ArgumentParser(description="Launch tuning curve jobs on Snellius")
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
    ...
    log_dir = (
        Path(args.output_dir) / "tuning_curves" / "logs"
        if args.output_dir
        else base_dir / "logs"
    )
    results_dir = (
        Path(args.output_dir) / "tuning_curves" / "results"
        if args.output_dir
    ...
    script_path = base_dir / "scripts" / "snellius" / "tuning_curves" / "FI_curve.py"

    initial_weights = np.linspace(0.0, 20.0, 20)
    balances = np.linspace(1.5, 3.0, 10)
    ...
    for iw in initial_weights:
        for balance in balances:
            for nl in noise_levels:
                ...
                name = f"FI_curve_iw{iw:.2f}_balance{balance:.2f}_nl{nl:.2f}"
                args = [
                    "--initial_weight",
                    str(iw),
                    "--balance",
