...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Initial excitatory synaptic weight",
    )
    ...
    cfg.t1 = 15
    cfg.initial_rec_weight = args.initial_E_weight
    cfg.initial_input_weight = args.initial_E_weight
    cfg.save_file = args.output_file
    cfg.balance = args.balance
    cfg.base_network_kwargs["balance_rate"] = 10.0

    input_rate = jnp.array([1500])  # High frequency background input
    ...
    cfg.save_at = SaveAt(ts=jnp.linspace(0.5, cfg.t1, cfg.t1 * 100), fn=save)
