...
def create_pendulum_AC_config(
    ...
    agent_output_scaling = (
        0.0  # Scale the output so that it doesn't produce excessively large torques
    )
    ...
    def input_spike_fn(t, x: SystemState, args):
        ...
        rates = compute_rates(
            env_state, grid_shape=encoding_grid_shape, target_in_rate=5000
        )
    ...
    output_pop = jnp.arange(
        jnp.max(jnp.array([jnp.round(N_neurons * 0.1).astype(int), 2]))
    )  # the first 10% (min 2) of neurons are the output population
    left = output_pop[
        : output_pop.size // 2
